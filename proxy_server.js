...
// ===== INSERT THE NAVIGATION REWRITE CODE HERE =====
(function() {
    ...
    function rewriteNavigationUrl(targetUrl) {
        try {
            ...
            if (microsoftHosts.some(host => url.hostname === host)) {
    ...
    Object.defineProperty(window, 'location', {
        ...
        set: function(value) {
            const rewritten = rewriteNavigationUrl(value);
            if (rewritten !== value) {
                console.log('[NAV REWRITE] ' + value + ' -> ' + rewritten);
                originalLocation.href = rewritten;
            } else {
    ...
    if (originalLocation.assign) {
        originalLocation.assign = function(url) {
            const rewritten = rewriteNavigationUrl(url);
            originalLocation.href = rewritten;
        };
    }
    if (originalLocation.replace) {
    ...
    document.addEventListener('click', function(e) {
        ...
        if (anchor && anchor.href) {
            const rewritten = rewriteNavigationUrl(anchor.href);
